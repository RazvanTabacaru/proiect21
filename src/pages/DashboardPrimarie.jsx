...
export default function DashboardPrimarie() {
  ...
  return (
    <div style={{ padding: 32, maxWidth: 900, margin: '0 auto', background: 'var(--culoare-bg)', minHeight: '100vh' }}>
      ...
      {tab === 'cereri' && (
        <>
          <div style={{ background: 'var(--culoare-bg-card)', border: `1px solid var(--culoare-border)`, borderRadius: 12, padding: 24, marginBottom: 32 }}>
            ...
            <form onSubmit={adaugaCerere}>
              <div style={{ marginBottom: 14 }}>
                ...
                <input value={form.titlu} onChange={e => setForm({ ...form, titlu: e.target.value })} style={inputStyle} required />
              </div>
              <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <select value={form.furnizor_id} onChange={e => setForm({ ...form, furnizor_id: e.target.value })} style={inputStyle}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => exportCereriPDF(cereri, profil?.nume)} style={{ ...btnStyle('var(--culoare-danger)'), padding: '8px 18px', fontSize: 14 }}>📄 PDF</button>
              <button onClick={() => exportCereriExcel(cereri, profil?.nume)} style={{ ...btnStyle('var(--culoare-success)'), padding: '8px 18px', fontSize: 14 }}>📊 Excel</button>
            </div>
          </div>
...
const inputStyle = { width: '100%', padding: '9px 12px', border: `1px solid var(--culoare-border-input)`, borderRadius: 8, fontSize: 14, boxSizing: 'border-box', background: 'var(--culoare-bg-input)', color: 'var(--culoare-text)' }
const btnStyle = (bg) => ({ background: bg, color: '#fff', border: 'none', borderRadius: 6, padding: '8px 20px', cursor: 'pointer', fontSize: 14 })